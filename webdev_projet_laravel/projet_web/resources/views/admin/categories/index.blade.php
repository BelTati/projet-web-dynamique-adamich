...
<div class="p-6 max-w-6xl mx-auto">
    ...
    <!-- SECTION 1 : SUGGESTIONS (MODÉRATION) -->
    @if($pending->count() > 0)
    <div class="bg-yellow-50 p-6 rounded-lg mb-8 border border-yellow-200 shadow-sm">
        <h2 class="text-lg font-bold text-yellow-800 mb-4 flex items-center">
            <span class="mr-2">🔔</span> Suggestions de prestataires
        </h2>
        <table class="w-full text-left">
            @foreach($pending as $cat)
            <tr class="border-b border-yellow-100 last:border-0">
                <td class="py-3">
                    <span class="font-semibold text-gray-700">{{ $cat->nom }}</span>
                ...
                <td class="py-3 text-right">
                    <form action="{{ route('admin.categories.update', $cat->id) }}" method="POST" class="inline">
                        ...
                        <button class="bg-green-600 hover:bg-green-700 text-white px-4 py-1 rounded text-sm font-bold transition">
                            Valider
                        </button>
                    </form>
                </td>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr>
                    ...
                    <td class="py-4 text-right">
                        ...
                        <form action="{{ route('admin.categories.destroy', $cat->id) }}" method="POST" class="inline-flex items-center gap-2">
                            @csrf @method('DELETE')
